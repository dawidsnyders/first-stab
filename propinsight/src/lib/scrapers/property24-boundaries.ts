...
export async function extractProperty24Boundary(
  ...
  try {
    ...
    const searchUrl = `https://www.property24.com/for-sale/${areaName
      .toLowerCase()
      .replace(/\s+/g, "-")}/western-cape/1`;
    ...
    console.log(
      `[Property24 Boundaries] Extracting boundary for ${areaName}...`
    );
    ...
    console.error(
      `[Property24 Boundaries] Error extracting boundary for ${areaName}:`,
      error
    );
    return null;
  }
