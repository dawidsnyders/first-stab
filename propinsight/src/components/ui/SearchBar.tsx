...
export function SearchBar({ onMapClick }: SearchBarProps) {
  ...
  return (
    <div className="relative w-full max-w-2xl">
      {/* Quick Select Areas */}
      <div className="mb-4">
        <div className="flex flex-nowrap gap-2.5 overflow-x-auto scrollbar-hide" style={{ padding: '2px', margin: '-2px' }}>
          {popularAreas.map((area) => (
            <motion.button
              ...
              className="group relative flex-shrink-0 px-5 py-3 text-sm font-medium rounded-xl bg-white text-stone-900 border border-stone-200/80 hover:border-sage-300/60 hover:shadow-lg hover:shadow-sage-100/50 active:shadow-md"
              style={{ transition: 'all 0.1s ease-out' }}
            >
              <span className="relative z-10 whitespace-nowrap">{area.name}</span>
              <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-white to-stone-50/50 opacity-0 group-hover:opacity-100" style={{ transition: 'opacity 0.1s ease-out' }} />
            </motion.button>
          ))}
        </div>
      </div>
